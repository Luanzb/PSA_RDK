...
count = 1;
    ...
    addpath(genpath(data_path));
        ...
        %%
        matrix(valor(1,1):valor(1,2),:) = [info.matrix(61:420,:); info.matrix(481:end,:)];

        sac_side = zeros(720,1);
        sac_side(matrix(:,4)==1 & s.macrosacvec(:,4) < 0,1) = 1;
        sac_side(matrix(:,4)==2 & s.macrosacvec(:,4) > 0,1) = 1;


        % data filtered by saccade accuracy (saccade error <= 2.6)

        sac_acc = hypot(RDK.EccDVA - abs(s.macrosacvec(:,4)), abs(s.macrosacvec(:,5))) >= 0 ...
            & hypot(RDK.EccDVA - abs(s.macrosacvec(:,4)), abs(s.macrosacvec(:,5))) <= 3.5;


        % data filtered by saccade latencies between 165 and 350 ms.

        sac_lat = s.macrosacvec(:,1) >= sacc_on & s.macrosacvec(:,1) <= sacc_off;

        sac_offset = s.macrosacvec(:,2) >= 200;
        ...
        sac(valor(1,1):valor(1,2),1) = sac1;

        macvec(valor(1,1):valor(1,2),:) = s.macrosacvec;
        %% proportion correct responses (discrimination)

        resp2(valor(1,1):valor(1,2),:) =  [resp(61:420,:); resp(481:end,:)];

        resp3 = resp2(:,1) == matrix(:,6);
        ...
        pc(1,1) = (sum(pre_pc1) / size(pre_pc1,1)); % val high
        pc(1,2) = (sum(pre_pc2) / size(pre_pc2,1)); % val low
        pc(1,3) = (sum(pre_pc3) / size(pre_pc3,1)); % inval high
        pc(1,4) = (sum(pre_pc4) / size(pre_pc4,1)); % inval low


%% sensitivity (d' rpime)
        

    dprime(1,1) = sqrt(2) * norminv(pc(1,1));
    dprime(1,2) = sqrt(2) * norminv(pc(1,2));
    dprime(1,3) = sqrt(2) * norminv(pc(1,3));
    dprime(1,4) = sqrt(2) * norminv(pc(1,4));

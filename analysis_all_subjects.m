...
for subject1 = 1:15
    ...
    for ses = 1:2
        ...
        pc1(subject1,1) = (sum(pre_pc1) / size(pre_pc1,1)); % val high
        pc1(subject1,2) = (sum(pre_pc2) / size(pre_pc2,1)); % val low
        pc1(subject1,3) = (sum(pre_pc3) / size(pre_pc3,1)); % inval high
        pc1(subject1,4) = (sum(pre_pc4) / size(pre_pc4,1)); % inval low


%% sensitivity (d' rpime)
        

    dprime1(subject1,1) = sqrt(2) * norminv(pc1(subject1,1));
    dprime1(subject1,2) = sqrt(2) * norminv(pc1(subject1,2));
    dprime1(subject1,3) = sqrt(2) * norminv(pc1(subject1,3));
    dprime1(subject1,4) = sqrt(2) * norminv(pc1(subject1,4));
    ...
    vec(subject1).macv = macvec1; 

    count = count+ 1;

end

macvec = [vec(1).macv;vec(2).macv;vec(3).macv;vec(4).macv;vec(5).macv;
          vec(6).macv;vec(7).macv;vec(8).macv;vec(9).macv;vec(10).macv;
          vec(11).macv;vec(12).macv;vec(13).macv;vec(14).macv];
dprime = dprime1;
pc = pc1;

%%

% Example: replace this with your actual 14x4 dataset
data = pc;  
% Preallocate logical matrix for outliers
outliers = false(size(data));
% Loop through each column (each condition)
for col = 1:size(data,2)
    ...
    % Detect outliers
    outliers(:,col) = (x < lower_bound) | (x > upper_bound);
    
end

% Optional: extract outlier values
...
plot3 = bar([1 2],[mean(dprime(:,1:2),1);mean(dprime(:,3:4),1)],LineWidth=1.5,BarWidth=.4);
...
error = (std(dprime) / sqrt(length(dprime)));
...
errorbar(x_coords, mean(dprime), error, 'Color',[.4 .4 .4], 'LineStyle', 'none', 'CapSize', 0,'LineWidth',1.5);

hold on;

scatter([repelem(.85,subject1)], dprime(:,1),20,'filled','o','MarkerFaceColor',[205,101,0]/255)
scatter([repelem(1.15,subject1)], dprime(:,2),20,'filled','o','MarkerFaceColor',[0 0 0])
scatter([repelem(1.85,subject1)], dprime(:,3),20,'filled','o','MarkerFaceColor',[205,101,0]/255)
scatter([repelem(2.15,subject1)], dprime(:,4),20,'filled','o','MarkerFaceColor',[0 0 0])
...
plot3 = bar([mean(mean(dprime(:,1:2),1)) mean(mean(dprime(:,3:4),1))],LineWidth=1.5,BarWidth=.4);
...
plot3 = bar([mean(mean(dprime(:,[1 3]),1)) mean(mean(dprime(:,[2 4]),1))],LineWidth=1.5,BarWidth=.4);
plot3.EdgeColor = [205,101,0]/255;
plot3.FaceColor = [205,101,0]/255;
...
[ dmap, limits,fudge ] =dataDensity(rmmissing(macvec(:,4)),rmmissing(macvec(:,5)),1920,1080,[-26.6667 26.6667 -15 15],.0001);
...
plot3 = bar([1 2],[mean(pc(:,1:2),1)*100;mean(pc(:,3:4),1)*100],LineWidth=1.5,BarWidth=.4);
plot3(1).EdgeColor = [205,101,0]/255;
plot3(1).FaceColor = [205,101,0]/255;
plot3(2).FaceColor = [0 0 0];
...
plot3 = bar([mean(mean(pc(:,[1 2]),1))*100 mean(mean(pc(:,[3 4]),1))*100],LineWidth=1.5,BarWidth=.4);
plot3.EdgeColor = [205,101,0]/255;
plot3.FaceColor = [205,101,0]/255;
...
plot3 = bar([mean(mean(pc(:,[1 3]),1))*100 mean(mean(pc(:,[2 4]),1))*100],LineWidth=1.5,BarWidth=.4);
plot3.EdgeColor = [205,101,0]/255;
plot3.FaceColor = [205,101,0]/255;
